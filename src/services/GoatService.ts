...
class GoatService {
  ...
  async createGoat(goat: Omit<Goat, "id">): Promise<boolean> {
    const filterGoat = {
      ...
      vaccinationDate: this.toMySQLDateTimeString(goat.vaccinationDate),
      ...goat,
    };
  ...
  async updateGoat(id: number, newData: Omit<Goat, "id">): Promise<boolean> {
    const filterGoat = {
      ...
      vaccinationDate: this.toMySQLDateTimeString(newData.vaccinationDate),
      ...newData,
    };
  ...
  async getGoats(
    ...
    if (res.status === 200) {
      goats = res.data.data.map((g: any) => {
        if (g.weightDate) g.weightDate = new Date(g.weightDate);
        if (g.birthDate) g.birthDate = new Date(g.birthDate);
        if (g.releaseDate) g.releaseDate = new Date(g.releaseDate);
        if (g.vaccinationDate) g.vaccinationDate = new Date(g.vaccinationDate);
